...
def start_mcbe_server(container_name):
    if not is_container_running(container_name):
        ...
        logger.info(f"Minecraft Bedrock server {container_name} initiated startup.")
        return True
    return False
        ...
        try:
            ...
            for s in readable:
                if s in client_listen_sockets.values():
                    ...
                    # <<< START OF FIX FOR CLIENT SWITCHING SERVERS >>>
                    # If client_addr is already known, check if they switched target server (by listen_port)
                    if client_addr in active_client_connections:
                        existing_conn_info = active_client_connections[client_addr]
                        if existing_conn_info["listen_port"] != listen_port:
                            logger.info(f"Client {client_addr} switched target server from port {existing_conn_info['listen_port']} to {listen_port}. Re-establishing session to new server.")
                            # Clean up the old connection socket and associated client tracking
                            inputs.remove(existing_conn_info["client_to_server_socket"])
                            existing_conn_info["client_to_server_socket"].close()
                            if client_addr in clients_per_server[existing_conn_info["target_container"]]:
                                clients_per_server[existing_conn_info["target_container"]].remove(client_addr)
                            del active_client_connections[client_addr] # Treat as a new connection below

                    # Now, whether it was new or just cleared due to port switch, process as a new connection
                    if client_addr not in active_client_connections:
                    # <<< END OF FIX FOR CLIENT SWITCHING SERVERS >>>

                        server_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                        server_sock.setblocking(False)

                        active_client_connections[client_addr] = {
                            "target_container": container_name, # This is correctly set based on the *current* listen_port
                            "client_to_server_socket": server_sock,
                            "last_packet_time": time.time(),
                            "listen_port": listen_port # Store the current listen_port for future checks
                        }
                        inputs.append(server_sock)
                        clients_per_server[container_name].append(client_addr)
                        logger.info(f"New client {client_addr} established session with {container_name} via port {listen_port}")
                        ...
                        for buffered_packet in packet_buffers[(client_addr, listen_port)]:
                            try:
                                ...
                                logger.error(f"Error sending buffered packet for {client_addr} to {container_name}: {e}")
                        ...
                        try:
                            ...
                            logger.error(f"Error sending initial packet for {client_addr} to {container_name}: {e}")

                    else: # It's an *existing* connection to the *same* server/port (after check above)
                        ...
                        if not target_server_config:
                            logger.error(f"Target server config not found for {target_container} for existing connection {client_addr}. Cleaning up.")
                        ...
                        try:
                            ...
                            logger.debug(f"Forwarded packet from {client_addr} to {target_container}")
                        except socket.error as e:
                            logger.error(f"Error forwarding packet from {client_addr} to {target_container}: {e}. Disconnecting client.")
                            inputs.remove(server_sock)
                            server_sock.close()
                            if client_addr in clients_per_server[target_container]:
                                clients_per_server[target_container].remove(client_addr)
                    ...
                    if found_client_addr:
                        ...
                        try:
                            ...
                            conn_info["last_packet_time"] = time.time()
                            logger.debug(f"Forwarded response from {conn_info['target_container']} to {found_client_addr}")
                        except socket.error as e:
                            logger.error(f"Error sending response from server to client {found_client_addr}: {e}. Disconnecting client.")
                        ...
                        logger.error(f"Received unexpected data on backend socket {s}. Client not found. Closing socket.")
                        inputs.remove(s)
                        s.close()
