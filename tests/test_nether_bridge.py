...
def test_wait_for_server_query_ready_timeout(mock_bedrock_lookup, mock_sleep, nether_bridge_instance, mock_servers_config):
    bedrock_config = next(s for s in mock_servers_config if s.server_type == 'bedrock')
    result = nether_bridge_instance._wait_for_server_query_ready(bedrock_config, 0.2, 0.1)
    assert result is False
    assert mock_bedrock_lookup.call_count > 1

@patch('nether_bridge.NetherBridgeProxy._stop_minecraft_server')
@patch('mcstatus.BedrockServer.lookup')
def test_monitor_stops_idle_server(mock_lookup, mock_stop, nether_bridge_instance, mock_servers_config):
    container_name = "test-mc-bedrock"
    state = nether_bridge_instance.server_states[container_name]
    state["running"] = True
    state["last_activity"] = time.time() - 20 # 20 seconds ago
    
    mock_status = MagicMock(players=MagicMock(online=0))
    mock_lookup.return_value.status.return_value = mock_status

    with patch('nether_bridge.time.sleep', side_effect=InterruptedError):
...
def test_monitor_resets_timer_for_active_server(mock_lookup, mock_stop, nether_bridge_instance, mock_servers_config):
