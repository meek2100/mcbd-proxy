...
def mock_app_config():
    ...
    config = MagicMock()
    server = MagicMock(name="test_server", container_name="test_container")
...
def metrics_manager(
    mock_total_gauge,
    mock_status_gauge,
    mock_connections_gauge,
    mock_app_config,
    mock_docker_manager,
):
    """Fixture for a MetricsManager instance, with gauges patched."""
    return MetricsManager(mock_app_config, mock_docker_manager)


def test_metrics_manager_init_disabled(mock_docker_manager):
    """Tests that no gauges are initialized when Prometheus is disabled."""
    config = MagicMock(is_prometheus_enabled=False)
    manager = MetricsManager(config, mock_docker_manager)
    assert not manager._is_enabled
...
async def test_update_server_status_periodically(
    ...
    """
    mock_docker_manager.is_container_running.side_effect = [True, False]
    with patch("asyncio.sleep", side_effect=[None, asyncio.CancelledError]):
        try:
            await metrics_manager._update_server_status_periodically()
        except asyncio.CancelledError:
            pass  # Expected to exit the loop for the test

    # First iteration: server is running
    mock_status_gauge.labels.return_value.set.assert_any_call(1)
    mock_total_gauge.set.assert_any_call(1)

    # Second iteration: server is not running
    mock_status_gauge.labels.return_value.set.assert_any_call(0)
    mock_total_gauge.set.assert_any_call(0)
